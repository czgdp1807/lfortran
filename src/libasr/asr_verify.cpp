...
#define require(cond, error_msg) require_impl((cond), (error_msg), x.base.base.loc)
    ...
    void visit_Var(const Var_t &x) {
        ...
        current_symtab->resolve_symbol("enum");
        SymbolTable *enum_symtab = nullptr;
        if (current_symtab->resolve_symbol("enum")) {
            ASR::symbol_t *enum_s = current_symtab->resolve_symbol("enum");
            enum_symtab = ASR::down_cast<ASR::EnumType_t>(enum_s)->m_symtab;
        }
        require(symtab_in_scope(current_symtab, x.m_v)
             || symtab_in_scope(enum_symtab, x.m_v),
